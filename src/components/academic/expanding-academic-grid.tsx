...
import { ACADEMIC_PLATFORMS, type AcademicPlatformItem } from './academic-data';
import { AcademicCard } from './academic-card';
import { AcademicExpandedDetail } from './academic-expanded-detail';

export function ExpandingAcademicGrid() {
  const [selectedCategory, setSelectedCategory] = useState<string>('Semua');
  const [searchQuery, setSearchQuery] = useState<string>('');
  const [activeItem, setActiveItem] = useState<AcademicPlatformItem | null>(null);
  ...
  const categories = useMemo(() => {
    ...
    set.add('Semua');
    ACADEMIC_PLATFORMS.forEach((item) => set.add(item.kategori));
    return Array.from(set);
  }, []);

  const filteredPlatforms = useMemo(() => {
    return ACADEMIC_PLATFORMS.filter((item) => {
  ...
  // Anime.js entrance stagger on filter/search change
  useEffect(() => {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="relative w-full max-w-xl flex items-center gap-3 rounded-2xl border border-sand bg-cream px-4 py-3 shadow-sm focus-within:border-rust focus-within:ring-2 focus-within:ring-rust/20 transition-all">
          ...
          <input
            ...
            placeholder="Cari portal iGracias, LMS, OpenLib..."
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filteredPlatforms.map((item, index) => (
          ...
          >
            <AcademicCard item={item} onClick={() => setActiveItem(item)} />
          </div>
        ))}
      </div>
      ...
      {filteredPlatforms.length === 0 && (
        <div className="py-16 text-center rounded-3xl border border-dashed border-sand/60 bg-cream/50">
          ...
          <h3 className="font-display text-xl font-bold text-forest-deep">
            Platform Akademik Tidak Ditemukan
          </h3>
          ...
          <AnimatePresence>
            {activeItem && (
              ...
              >
                ...
                >
                  <AcademicExpandedDetail item={activeItem} onClose={() => setActiveItem(null)} />
                </motion.div>
              </motion.div>
            )}
          </AnimatePresence>,
